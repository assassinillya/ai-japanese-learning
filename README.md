# ai-japanese-learning v1.4

当前版本已经推进到 `v1.4`，重点收敛上传后题目预生成、生词复习题预热、复习交互即时反馈、AI 配置保存和前端布局细节，同时继续保留公共文章库、可拖动查词弹窗、AI 例句管理和多供应商 AI 接入能力。
...
- 上传文章后会异步生成挑战阅读题和阅读后测验题，前端用双进度条控制入口可用状态
- 挑战阅读、阅读后测验和词汇复习选项统一为卡片式 ABCD 选项
